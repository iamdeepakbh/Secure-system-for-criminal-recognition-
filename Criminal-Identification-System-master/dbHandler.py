...
def insertData(data):
    ...
    db = pymysql.connect(host="localhost",user= "root",password= "root",database="criminaldb3")
    cursor = db.cursor()
    ...
    query = "INSERT INTO criminaldata VALUES(0, '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % \
            (data["Name"], data["Father's Name"], data["Mother's Name"], data["Gender"],
             data["DOB(yyyy-mm-dd)"], data["Aadhaar Number"], data["Crime Type"],
             data["Nationality"], data["Religion"], data["Area"])
